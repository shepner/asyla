...
if [ -d "$path/unifi-network-application" ]; then
    ...
    doas rm -rf "$path/unifi-network-application"
fi
echo ""
...
doas mkdir -p "$path/unifi-network-application/config"
doas mkdir -p "$path/unifi-network-application/db"
doas chown -R 1003.1000  "$path/unifi-network-application"
...
#mkdir -p "$tmp_dir"
#cd "$tmp_dir"
cd "$path/unifi-network-application"
...
cp ~/scripts/docker/unifi-network-application/docker-compose.yml .
...
for container_name in "${container_names[@]}"; do
    ...
    if doas docker ps -a --format '{{.Names}}' | grep -q "^$container_name$"; then
        ...
        doas docker stop "$container_name" >/dev/null 2>&1
        #docker rm "$container_name" >/dev/null 2>&1
        doas docker rm "$container_name" >/dev/null 2>&1
        ...
        doas docker volume rm "$(docker volume ls -qf "name=${container_name}_data")" >/dev/null 2>&1
    fi
done
