#!/bin/bash
# Full build script for d02 VM
#
# Purpose: Destroy and recreate VM 102 on Proxmox, import Debian cloud image,
#          configure cloud-init (vendor runs install), and verify SSH and software.
# Usage:   ./d02/build.sh   (from repo root or d02/)
# Requires: SSH access to root@vmh02, d02 in ~/.ssh/config (HostName 10.0.0.61, User docker),
...
# Configuration - paths relative to this script so build works from repo root or d02/
...
SNIPPETS_PATH="/var/lib/vz/snippets/d02-cloud-init.yml"
VM_HOST="d02"
VM_SSH_OPTS="-o StrictHostKeyChecking=no"

log_step "Starting Full Build Test for d02 VM"
...
scp "$CLOUD_INIT_VENDOR_FILE" "root@$PROXMOX_HOST:/var/lib/vz/snippets/d02-cloud-init-vendor.yml" || {
...
qm set $VMID --ipconfig0 ip=10.0.0.61/24,gw=10.0.0.1 && \
qm set $VMID --nameserver '10.0.0.10 10.0.0.11' && \
qm set $VMID --searchdomain asyla.org && \
qm set $VMID --cicustom vendor=local:snippets/d02-cloud-init-vendor.yml && \
...
log_info "Waiting for SSH (d02) - checking once per minute, max 20 min..."
...
for i in $(seq 1 18); do
  if ssh $VM_SSH_OPTS -o ConnectTimeout=10 "$VM_HOST" "test -f ~/scripts/d02/setup/docker.sh && command -v docker >/dev/null 2>&1" 2>/dev/null; then
...
if ssh $VM_SSH_OPTS -o ConnectTimeout=5 "root@${VM_HOST}" "command -v cloud-init >/dev/null 2>&1" 2>/dev/null; then
    ...
    log_info "From VM console (as root), run: curl -s https://raw.githubusercontent.com/shepner/asyla/master/d02/setup/bootstrap.sh | bash"
fi

# Step 15: Test SSH access
log_step "Step 15: Test SSH access"
log_info "Attempting SSH connection to d02..."
if ssh $VM_SSH_OPTS -o ConnectTimeout=10 "$VM_HOST" "echo 'SSH connection successful!'" 2>/dev/null; then
    ...
    log_warn "⚠️  VM is using DHCP IP instead of static 10.0.0.61"
else
    log_warn "SSH not ready yet - cloud-init may still be processing"
    log_info "Check console or try: ssh d02 (or ssh root@10.0.0.248 if using DHCP)"
fi
...
if $SSH_VERIFY "echo ok" 2>/dev/null; then
  ...
  HAS_SCRIPTS=$($SSH_VERIFY "test -f ~/scripts/d02/setup/docker.sh && echo y" 2>/dev/null) || true
  DOCKER_VER=$($SSH_VERIFY "docker --version 2>/dev/null" 2>/dev/null) || true
  if [ "$HOSTNAME" = "d02" ]; then
    log_info "✅ Hostname is d02 (update_scripts.sh can find d02/ in repo)"
  else
    log_warn "Hostname is '$HOSTNAME' (expected d02) - scripts may not have been installed"
  fi
  if [ "$HAS_SCRIPTS" = "y" ]; then
    log_info "✅ Setup scripts installed at ~/scripts/d02/setup/"
  else
    log_warn "Setup scripts not found at ~/scripts/d02/setup/ - update_scripts.sh may have failed"
  fi
  ...
  $SSH_VERIFY "id; groups; ls ~/scripts/d02/setup/ 2>/dev/null | head -15" 2>/dev/null || true
else
  log_warn "Could not verify - SSH not ready. Try: ssh d02"
fi
...
if ! ssh $VM_SSH_OPTS -o ConnectTimeout=5 "$VM_HOST" "echo ok" 2>/dev/null; then
  ...
  if $SSH_VERIFY "echo ok" 2>/dev/null; then
    ...
    HAS_SCRIPTS=$($SSH_VERIFY "test -f ~/scripts/d02/setup/docker.sh && echo y" 2>/dev/null) || true
    DOCKER_VER=$($SSH_VERIFY "docker --version 2>/dev/null" 2>/dev/null) || true
    [ "$HOSTNAME" = "d02" ] && log_info "✅ Hostname is d02" || log_warn "Hostname is '$HOSTNAME' (expected d02)"
    [ "$HAS_SCRIPTS" = "y" ] && log_info "✅ Setup scripts at ~/scripts/d02/setup/" || log_warn "Setup scripts not found"
    ...
    $SSH_VERIFY "ls ~/scripts/d02/setup/ 2>/dev/null | head -15" 2>/dev/null || true
  else
    log_warn "SSH still not ready. Try manually in a few min: ssh d02"
  fi
fi
...
if ssh $VM_SSH_OPTS -o ConnectTimeout=5 "$VM_HOST" "test -d /home/docker" 2>/dev/null; then
  ...
  if [ -t 0 ]; then
    ...
    DO_SMB=""
    log_info "No TTY - skipping SMB prompt. To set later: ssh d02, run ~/setup_manual.sh"
  fi
  if echo "${DO_SMB}" | grep -qi '^y'; then
    ...
    if [ -n "$SMB_USER" ] && [ -n "$SMB_PASS" ]; then
      ...
      if scp -o StrictHostKeyChecking=no "$SMB_TMP" "${VM_HOST}:/tmp/smbcreds" 2>/dev/null; then
        ...
        log_warn "Could not copy credentials to VM (SSH/SCP failed). Set manually: ssh d02, run ~/setup_manual.sh"
      fi
      rm -f "$SMB_TMP"
    else
      log_info "Skipped (empty username or password). Set later on d02: ~/setup_manual.sh"
    fi
  else
    log_info "Skipped. To set later: ssh d02, run ~/setup_manual.sh"
  fi
else
  log_info "SSH not available; set SMB later: ssh d02, run ~/setup_manual.sh"
fi

echo ""
log_info "Next steps:"
echo "  1. SSH to VM: ssh d02"
echo "  2. Verify configuration: id docker && groups docker"
echo "  3. Copy SSH keys for docker-to-docker access:"
...
echo "     ssh d02 'chmod 600 ~/.ssh/docker_rsa ~/.ssh/config && chmod 644 ~/.ssh/docker_rsa.pub && chmod 700 ~/.ssh'"
echo "  4. Run SSH setup script: ssh d02 '~/scripts/d02/setup/setup_ssh_keys.sh'"
echo "  5. Add d02's public key to d01 and d03 (for app migration):"
echo "     ssh d02 'cat ~/.ssh/docker_rsa.pub' | ssh d01 'cat >> /home/docker/.ssh/authorized_keys && chmod 600 /home/docker/.ssh/authorized_keys'"
echo "     ssh d02 'cat ~/.ssh/docker_rsa.pub' | ssh d03 'cat >> /home/docker/.ssh/authorized_keys && chmod 600 /home/docker/.ssh/authorized_keys'"
echo "  6. Test docker-to-docker SSH: ssh d02 'ssh docker@d01 echo test'"
echo "  7. Manual SMB credentials: ~/setup_manual.sh"
echo ""
log_info "Note: For app migration script to work, SSH keys must be configured for docker-to-docker access."
log_info "      See d02/README.md for detailed instructions."
