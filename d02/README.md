...
- **VM Name**: d02
- **Hostname**: d02.asyla.org
- **IP Address**: 10.0.0.61/24
- **Gateway**: 10.0.0.1
...
**Note:** Before removing the old d02, consider cloning it to a backup VMID (e.g., 199) to keep it as a backup:
```bash
# Clone old d02 to backup VMID
OLD_VMID=102
BACKUP_VMID=199
BACKUP_NAME="d02-backup"
qm shutdown $OLD_VMID
qm clone $OLD_VMID $BACKUP_VMID --name $BACKUP_NAME --full
```

```bash
# From Proxmox host (vmh01 or vmh02) as root
VMID=102  # Verify this is the correct VMID for old d02
...
scp d02/setup/cloud-init-vendor.yml root@vmh02:/var/lib/vz/snippets/d02-cloud-init-vendor.yml
...
qm set $VMID \
  ...
  --ipconfig0 ip=10.0.0.61/24,gw=10.0.0.1 \
  --nameserver '10.0.0.10 10.0.0.11' \
  --searchdomain asyla.org \
  --cicustom vendor=local:snippets/d02-cloud-init-vendor.yml
...
- **IP Address**: `10.0.0.61` (configured via Proxmox built-in cloud-init)
...
2. SSH to the VM using your SSH key:
   ...
   **After a rebuild:** The VM gets a new SSH host key. Edit `~/.ssh/known_hosts` on your workstation and remove the old line for `10.0.0.61` or `d02`, then connect again (or answer `yes` when SSH asks to accept the new key).
...
To enable SSH to other docker hosts (d01, d03) using the same identity:

```bash
# From your workstation
scp ~/.ssh/docker_rsa d02:.ssh/docker_rsa
scp ~/.ssh/docker_rsa.pub d02:.ssh/docker_rsa.pub
scp ~/.ssh/config d02:.ssh/config
ssh d02 "chmod 600 ~/.ssh/docker_rsa ~/.ssh/config && chmod 644 ~/.ssh/docker_rsa.pub && chmod 700 ~/.ssh"

# Verify setup
ssh d02 '~/scripts/d02/setup/setup_ssh_keys.sh'
```
...
1. **Copy SSH keys to d02** (from workstation):
   ```bash
   scp ~/.ssh/docker_rsa d02:.ssh/docker_rsa
   scp ~/.ssh/docker_rsa.pub d02:.ssh/docker_rsa.pub
   scp ~/.ssh/config d02:.ssh/config
...
2. **Add d02's public key to d01 and d03** (so d02 can SSH to them):
   ```bash
   # From workstation, copy d02's public key to d01 and d03
   ssh d02 "cat ~/.ssh/docker_rsa.pub" | ssh d01 "cat >> /home/docker/.ssh/authorized_keys && chmod 600 /home/docker/.ssh/authorized_keys"
   ssh d02 "cat ~/.ssh/docker_rsa.pub" | ssh d03 "cat >> /home/docker/.ssh/authorized_keys && chmod 600 /home/docker/.ssh/authorized_keys"
   ```

3. **Test connectivity**:
   ```bash
   ssh d02 'ssh docker@d01 echo "SSH to d01 works"'
   ssh d02 'ssh docker@d03 echo "SSH to d03 works"'
   ```

**Alternative: Use setup script**
After copying keys to d02, run the setup script:
```bash
ssh d02 '~/scripts/d02/setup/setup_ssh_keys.sh'
```
...
**d02 uses docker-compose** (different from d01 which uses shell scripts).
...
- ⚠️ TODO: Determine which containers will run on d02
- d02-old had no containers defined - this is a fresh start
...
From the VM console (as root): `curl -s https://raw.githubusercontent.com/shepner/asyla/master/d02/setup/bootstrap.sh | bash`
...
**Symptoms:**
- VM gets IP like `10.0.0.248` instead of `10.0.0.61`
