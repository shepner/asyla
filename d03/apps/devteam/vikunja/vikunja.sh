#!/bin/bash
# Vikunja on d03. Usage: vikunja.sh [switch ...] e.g. backup|update|refresh|up|down|restart|logs
# Switches can be combined (e.g. down backup up). Run from anywhere; loads ~/scripts/docker/common.env.
# Data under /mnt/docker/Devteam/Vikunja; backups go to /mnt/nas/data1/docker (tgz).
...
APP_NAME="Vikunja"
APP_ROOT="$DOCKER_DL/Devteam/Vikunja"
...
do_backup() {
  ...
  tar -czf "$archive" -C "${DOCKER_DL}/Devteam" Vikunja
...
run_cmd() {
  ...
  case "$cmd" in
    ...
    pull)
      ...
      ;;
    up)
      mkdir -p "$APP_ROOT"/{files,pgdata}
      chown 1000 "$APP_ROOT/files" 2>/dev/null || true
      run_compose up -d
      ;;
    down)
      run_compose down
