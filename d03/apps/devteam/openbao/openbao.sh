#!/bin/bash
# OpenBao on d03. Usage: openbao.sh [switch ...] e.g. backup|update|refresh|up|down|restart|logs
# Switches can be combined (e.g. down backup up). Run from anywhere; loads ~/scripts/docker/common.env.
# Data under /mnt/docker/Devteam/OpenBao; backups go to /mnt/nas/data1/docker (tgz).
...
APP_NAME="OpenBao"
APP_ROOT="$DOCKER_DL/Devteam/OpenBao"
...
do_backup() {
  ...
  tar -czf "$archive" -C "${DOCKER_DL}/Devteam" OpenBao
...
run_cmd() {
  ...
  case "$cmd" in
    ...
    refresh)
      ...
      ;;
    up)
      mkdir -p "$APP_ROOT/file" "$APP_ROOT/logs"
      run_compose up -d
      ;;
    down)
      run_compose down
