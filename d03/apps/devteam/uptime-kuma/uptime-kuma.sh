#!/bin/bash
# Uptime Kuma on d03. Usage: uptime-kuma.sh [switch ...] e.g. backup|update|refresh|up|down|restart|logs
# Switches can be combined (e.g. down backup up). Run from anywhere; loads ~/scripts/docker/common.env.
# Data under /mnt/docker/Devteam/UptimeKuma; backups go to /mnt/nas/data1/docker (tgz).

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
COMPOSE_FILE="$SCRIPT_DIR/compose.yml"
SCREEN_APP="uptime-kuma"
...
APP_NAME="UptimeKuma"
APP_ROOT="$DOCKER_DL/Devteam/UptimeKuma"
...
do_backup() {
  ...
  tar -czf "$archive" -C "${DOCKER_DL}/Devteam" UptimeKuma
