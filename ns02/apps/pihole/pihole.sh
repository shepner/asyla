#!/bin/bash
# Pi-hole DNS server on ns02. Usage: pihole.sh [switch ...] e.g. backup|up|down|logs|refresh|update|restart
...
if [ $# -eq 0 ]; then
  ...
  echo "" >&2
  echo "  backup   - Create tgz of pihole-ns02 data to NFS (runs in screen)" >&2
