...
log_step "Starting Bootstrap for ns02 VM"
...
curl -s https://raw.githubusercontent.com/shepner/asyla/master/ns02/setup/cloud-init-userdata.yml > /var/lib/cloud/seed/nocloud/user-data || {
...
if ip addr show ens18 | grep -q "10.0.0.11"; then
    log_info "✅ Network configured correctly (10.0.0.11)"
else
    CURRENT_IP=$(ip addr show ens18 | grep "inet " | awk '{print $2}' | cut -d/ -f1)
    log_warn "⚠️  Network IP is $CURRENT_IP (expected 10.0.0.11)"
    log_info "Network may need manual configuration or reboot"
fi

log_step "Bootstrap Complete!"
log_info "Next steps:"
log_info "  1. SSH to VM: ssh docker@10.0.0.11"
log_info "  2. Copy SSH private key: scp ~/.ssh/docker_rsa ns02:.ssh/docker_rsa"
log_info "  3. Copy SSH config: scp ~/.ssh/config ns02:.ssh/config"
log_info "  4. Run setup scripts: ~/scripts/ns02/setup/*.sh"
