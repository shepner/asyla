...
# Run once initiator is in TrueNAS Initiator Group: ~/setup_manual.sh  (or sudo ~/scripts/ns02/setup/setup_iscsi_connect.sh)
...
NAME_OF_TARGET="iqn.2005-10.org.freenas.ctl:nas01:ns02:01"

echo "This host's initiator (must be in TrueNAS Initiator Group for target nas01:ns02:01):"
...
if ! iscsiadm -m discovery -t sendtargets -p "$IP_OF_TARGET" 2>/dev/null; then
    echo "Discovery returned no targets. Add the initiator above to TrueNAS Initiator Group, then re-run this script."
    exit 1
fi

echo "Logging in..."
if ! iscsiadm --mode node --targetname "$NAME_OF_TARGET" --portal "$IP_OF_TARGET" --login 2>/dev/null; then
    echo "Login failed. Add the initiator above to TrueNAS Initiator Group, then re-run: sudo $0"
    exit 1
fi
