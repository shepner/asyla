#!/bin/bash
# Full build script for d01 VM
#
# Purpose: Destroy and recreate VM 101 on Proxmox (vmh01), import Debian cloud image,
#          configure cloud-init (vendor runs install), and verify SSH and software.
# Usage:   ./d01/build.sh   (from repo root or d01/)
# Requires: SSH access to root@vmh01, d01 in ~/.ssh/config (HostName 10.0.0.60, User docker),
...
# Configuration - paths relative to this script so build works from repo root or d01/
...
SNIPPETS_PATH="/var/lib/vz/snippets/d01-cloud-init.yml"
VM_HOST="d01"
VM_SSH_OPTS="-o StrictHostKeyChecking=no"

log_step "Starting Full Build for d01 VM"
...
scp "$CLOUD_INIT_VENDOR_FILE" "root@$PROXMOX_HOST:/var/lib/vz/snippets/d01-cloud-init-vendor.yml" || {
...
qm set $VMID --ipconfig0 ip=10.0.0.60/24,gw=10.0.0.1 && \
qm set $VMID --nameserver '10.0.0.10 10.0.0.11' && \
qm set $VMID --searchdomain asyla.org && \
qm set $VMID --cicustom vendor=local:snippets/d01-cloud-init-vendor.yml && \
...
log_info "Waiting for SSH (d01) - checking once per minute, max 20 min..."
...
for i in $(seq 1 18); do
  if ssh $VM_SSH_OPTS -o ConnectTimeout=10 "$VM_HOST" "test -f ~/scripts/d01/setup/docker.sh && command -v docker >/dev/null 2>&1" 2>/dev/null; then
...
if ssh $VM_SSH_OPTS -o ConnectTimeout=10 "$VM_HOST" "echo 'SSH connection successful!'" 2>/dev/null; then
    ...
    log_info "Check console or try: ssh d01"
fi
...
if $SSH_VERIFY "echo ok" 2>/dev/null; then
  ...
  HAS_SCRIPTS=$($SSH_VERIFY "test -f ~/scripts/d01/setup/docker.sh && echo y" 2>/dev/null) || true
  DOCKER_VER=$($SSH_VERIFY "docker --version 2>/dev/null" 2>/dev/null) || true
  if [ "$HOSTNAME" = "d01" ]; then
    log_info "✅ Hostname is d01"
  else
    log_warn "Hostname is '$HOSTNAME' (expected d01)"
  fi
  if [ "$HAS_SCRIPTS" = "y" ]; then
    log_info "✅ Setup scripts installed at ~/scripts/d01/setup/"
  else
    log_warn "Setup scripts not found at ~/scripts/d01/setup/"
  fi
  ...
  $SSH_VERIFY "id; groups; ls ~/scripts/d01/setup/ 2>/dev/null | head -15" 2>/dev/null || true
else
  log_warn "Could not verify - SSH not ready. Try: ssh d01"
fi
...
log_info "Next steps:"
echo "  1. SSH to VM: ssh d01"
echo "  2. Copy SSH keys: scp ~/.ssh/docker_rsa d01:.ssh/docker_rsa"
echo "     scp ~/.ssh/docker_rsa.pub d01:.ssh/docker_rsa.pub"
echo "     scp ~/.ssh/config d01:.ssh/config"
echo "     ssh d01 'chmod 600 ~/.ssh/docker_rsa ~/.ssh/config && chmod 700 ~/.ssh'"
echo "  3. Run: ssh d01 '~/scripts/d01/setup/setup_ssh_keys.sh'"
echo "  4. Configure cloudflared: cp ~/scripts/d01/cloudflared/.env.example ~/scripts/d01/cloudflared/.env and set TUNNEL_TOKEN"
echo "  5. Start cloudflared and internal-proxy: see d01/cloudflared/README.md and d01/internal-proxy/README.md"
